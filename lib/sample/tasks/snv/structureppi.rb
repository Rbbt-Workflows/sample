...
module Sample
  ...
  dep Structure, :mi_interfaces, :mutated_isoforms => :ns_mutated_isoforms, :organism => :organism
  ...
  Structure::ANNOTATORS.keys.sort.collect do |database|
    ...
    dep Structure, :annotate_mi, :database => database.to_s, :mutated_isoforms => :ns_mutated_isoforms, :organism => :organism
  end
  ...
  Structure::ANNOTATORS.keys.sort.collect do |database|
    next if database == "COSMIC"
    dep Structure, :annotate_mi_neighbours, :database => database.to_s, :mutated_isoforms => :ns_mutated_isoforms, :organism => :organism
  end
